...
def plotClassify2D(learner, X, Y, pre=lambda x: x, axis=None, nGrid=128, **kwargs):
    ...
    if learner is not None:
        ...
        vmin = min( YGrid.max(), vmin )
        vmax = max( YGrid.max(), vmax )
        #axis.contourf( xticks,yticks,YGrid.reshape( (len(xticks),len(yticks)) ), nClasses )
        axis.imshow( YGrid.reshape( (len(xticks),len(yticks)) ), extent=ax, interpolation='nearest',origin='lower',alpha=0.5, aspect='auto' , vmin=vmin,vmax=vmax)
